...
const { ClassificationMaterial, validationClassficationMatrials } = require("../models/classificationMaterial");

// @desc    Create new classification material
// @route   POST /api/classificationMaterial
// @access  private
module.exports.createClassfiMaterial = asyncHandler(async (req, res) => {
  const { error } = validationClassficationMatrials(req.body);
  if (error) {
    return res.status(400).json({ message: error.details[0].message });
  }
  const classficationMaterial = await ClassificationMaterial.create({
...
// @route   GET /api/classificationMaterial
// @access  private
module.exports.getAllClassfiMaterial = asyncHandler(async (req, res) => {
  const materials = await ClassificationMaterial.find();
  res.status(200).json({
...
// @route   GET /api/classificationMaterial/:id
// @access  private
module.exports.getClassfiMaterialById = asyncHandler(async (req, res) => {
  const material = await ClassificationMaterial.findById(req.params.id).populate('materials');
  if (!material) {
...
// @route   PUT /api/classificationMaterial/:id
// @access  private
module.exports.updateClassfiMaterial = asyncHandler(async (req, res) => {
  ...
  const updatedMaterial = await ClassificationMaterial.findByIdAndUpdate(
    req.params.id,
    {
      $set: {
        fatherName: req.body.fatherName,
...
// @route   DELETE /api/classificationMaterial/:id
// @access  private
module.exports.deleteClassfiMaterial = asyncHandler(async (req, res) => {
  const material = await ClassificationMaterial.findByIdAndDelete(req.params.id);
  if (!material) {
    return res.status(404).json({ message: "Classification material not found" });
  }
  res.status(200).json({
